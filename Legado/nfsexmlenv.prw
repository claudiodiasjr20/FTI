	...
	If cTipo == "1" .And. Empty(cMotCancela)
		...
		If DbSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
           ...
           If cMvMsgTrib $ "1-3"
			...
			While !(cAliasSD2)->(Eof()) .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
				...
				//Chamado 37765 - Ordem de Informações da TAG <discr>/<Discriminacao>
				//Descrição do Serviço
				//Mensagem do SC5
				//Mensagem fixa "Vencimento da Nota" - "Total de Impostos"
				...
				//Customizacao - 37765 - Novo V11
				If dbSeek(xFilial("SX5")+"60"+IIF(SM0->M0_ESTCOB=="RJ", SB1->B1_XISSRJ, SB1->B1_CODISS ))
					If !lNFeDesc
						If nCont == 1 
				...
				If !lNFeDesc
					...
					ElseIf lNatOper .And. lSC5 .And. !Empty(SC5->C5_MENNOTA).And. nCont == 1
						//Customizacao - 37765 - Novo V11
						If cEmpAnt == "05"
							cNatOper += If(FindFunction('CleanSpecChar'),CleanSpecChar(Alltrim(SC5->C5_MENNOTA)+AllTrim(SC5->C5_XMENOT1)+AllTrim(SC5->C5_XMENOT2)),AllTrim(SC5->C5_MENNOTA)+AllTrim(SC5->C5_XMENOT1)+AllTrim(SC5->C5_XMENOT2))
						Else
							cNatOper += If(FindFunction('CleanSpecChar'),CleanSpecChar(Alltrim(SC5->C5_MENNOTA)),SC5->C5_MENNOTA)
						EndIf         
						cNatOper += "$$$"
						//Fim - 37765 - Novo V11
					ElseIf SF2->(FieldPos("F2_MENNOTA")) > 0
						If SF2->(FieldPos("F2_MENNOTA")) <> 0 .and. !AllTrim(SF2->F2_MENNOTA) $ cMensCli .and. !Empty(AllTrim(SF2->F2_MENNOTA))
             				cNatOper +=If(FindFunction('CleanSpecChar'),CleanSpecChar(AllTrim(SF2->F2_MENNOTA)),AllTrim(SF2->F2_MENNOTA))
						EndIf
					Else
						If IsBlind()
							MsgInfo ("Campo F2_MENNOTA inexistente, execute U_UPDSIGAFIS", "NFS-e")
						Else
							conout ("Campo F2_MENNOTA inexistente, execute U_UPDSIGAFIS")
						Endif
					EndIf

					//Customizacao - 37765 - Novo V11
					cNatOper += c_EOL
					cNatOper += c_EOL+"VENCIMENTO DA NOTA.: "+fGetVcto(aDupl)
					cNatOper += c_EOL+"TOTAL IMPOSTOS.....: R$ "+AllTrim(Transform(fGetImp(aRetido), "@E 999,999,999.99"))+c_EOL		
					cNatOper += c_EOL
					//Fim - 37765 - Novo V11
				Else 
					...
					ElseIf lSC5 .And. !Empty(SC5->C5_MENNOTA).And. nCont == 1
						//Customizacao - 37765 - Novo V11
						If cEmpAnt == "05"
							cDiscrNFSe += If(FindFunction('CleanSpecChar'),CleanSpecChar(Alltrim(SC5->C5_MENNOTA)+AllTrim(SC5->C5_XMENOT1)+AllTrim(SC5->C5_XMENOT2)),AllTrim(SC5->C5_MENNOTA)+AllTrim(SC5->C5_XMENOT1)+AllTrim(SC5->C5_XMENOT2))
						Else
					...
					//Customizacao - 37765 - Novo V11
					cDiscrNFSe += c_EOL
					cDiscrNFSe += c_EOL+"VENCIMENTO DA NOTA.: "+fGetVcto(aDupl)
					cDiscrNFSe += c_EOL+"TOTAL IMPOSTOS.....: R$ "+AllTrim(Transform(fGetImp(aRetido), "@E 999,999,999.99"))+c_EOL
					cDiscrNFSe += c_EOL
					//Fim - 37765 - Novo V11
				EndIf
				...
				If !lNFeDesc
					...
					ElseIf lNatOper .And. lSC5 .And. !Empty(SC5->C5_MENNOTA).And. nCont == 1
						...
						cNatOper += "$$$"
					ElseIf SF2->(FieldPos("F2_MENNOTA")) > 0
						IF !AllTrim(SF2->F2_MENNOTA) $ cMensCli .and. !Empty(AllTrim(SF2->F2_MENNOTA))
             				cDiscrNFSe +=If(FindFunction('CleanSpecChar'),CleanSpecChar(AllTrim(SF2->F2_MENNOTA)),AllTrim(SF2->F2_MENNOTA))
             			EndIF
					Else
						if IsBlind()
				...
				Else
					...
					ElseIf lSC5 .And. !Empty(SC5->C5_MENNOTA).And. nCont == 1
						...
						cDiscrNFSe += "$$$"
						ElseIf SF2->(FieldPos("F2_MENNOTA")) > 0
							If !Empty(AllTrim(SF2->F2_MENNOTA)) .And. nCont == 1
	             				cDiscrNFSe +=If(FindFunction('CleanSpecChar'),CleanSpecChar(AllTrim(SF2->F2_MENNOTA)),AllTrim(SF2->F2_MENNOTA))
	             			EndIF
						EndIf
				EndIf */
				...
				ElseIf !AllTrim(SC5->C5_MENNOTA) $ cMensCli
					If cEmpAnt == "05"
						cMensCli += If(FindFunction('CleanSpecChar'),CleanSpecChar(Alltrim(SC5->C5_MENNOTA)+AllTrim(SC5->C5_XMENOT1)+AllTrim(SC5->C5_XMENOT2)),AllTrim(SC5->C5_MENNOTA)+AllTrim(SC5->C5_XMENOT1)+AllTrim(SC5->C5_XMENOT2))
					Else
						cMensCli += If(FindFunction('CleanSpecChar'),CleanSpecChar(Alltrim(SC5->C5_MENNOTA)),SC5->C5_MENNOTA)
					EndIf
				EndIf
		...
		if !empty(aNota)
			...
			if len (aProd) > 0
				...
				cString	+= servicos( aProd, aISSQN, aRetido, cNatOper, lNFeDesc, cDiscrNFSe, aCST, aDest[22], SM0->M0_CODMUN, cF4Agreg ,nDescon,cFntCtrb,@aLeiTrp )
				cString	+= valores( aISSQN, aRetido, aTotal, aDest, SM0->M0_CODMUN,aLeiTrp )
...
/*/
//-----------------------------------------------------------------------
static function servicos( aProd, aISSQN, aRetido, cNatOper, lNFeDesc, cDiscrNFSe,aCST, cTpPessoa, cCodMun, cF4Agreg, nDescon, cFntCtrb,aLeiTrp )
	...
	for nX := 1 to len( aProd )
		...
		//Customizacao - 37765 - Novo V11
		ElseIf cCodMun $ "3304557"
			cString += '<codigo>'+Left(aProd[Nx][24],4)+'</codigo>'		
		//Fim - 37765 - Novo V11	
		Else
...
/*/
//-----------------------------------------------------------------------
static function valores( aISSQN, aRetido, aTotal, aDest, cCodMun,aLeiTrp )
...
static function infCompl( cMensCli, cMensFis, lNFeDesc, cDescrNFSe )
	...
	If !lNFeDesc
		If cEmpAnt == "05"
			cString	+= "<descricao>" + cMensCli
			If SF2->F2_VALIRRF > 0
				cString += " IRRF "+AllTrim(Transform(Round((SF2->F2_VALIRRF/SF2->F2_VALBRUT)*100,1),'@E 99.99'))+"% = R$ "+AllTrim(Transform(SF2->F2_VALIRRF,'@E 9,999,999.99'))
			EndIf
			If SF2->F2_VALPIS > 0
				cString += " CRF "+AllTrim(Transform(Round(((SF2->F2_VALCSLL+SF2->F2_VALCOFI+SF2->F2_VALPIS)/SF2->F2_VALBRUT)*100,2),'@e 99.99'))+"% = R$ "+AllTrim(Transform(SF2->F2_VALCSLL+SF2->F2_VALCOFI+SF2->F2_VALPIS,'@e 9,999,999.99'))
			EndIf
			cString += space( 1 ) + cMensFis + "</descricao>"
		Else
...
Static Function fGetVcto(a_Dupl)

Local n_Dup 	:= 0
Local c_RetVcto	:= ""

For n_Dup := 1 To Len(a_Dupl)

	If ( n_Dup > 1 )
		c_RetVcto += ", "
	Endif
	
	c_RetVcto += DTOC(a_Dupl[n_Dup][2])
...
Static Function fGetImp(a_Retido)

Local n_Imp			:= 0    
Local n_RetVlrImp	:= 0
                    
For n_Imp := 1 To Len(a_Retido)

	n_RetVlrImp += a_Retido[n_Imp][3]
