...
±±ºPrograma  ³ RLFINR01()	 º Autor ³ Tiago Dias   º Data ³  22/09/15   	 º±±
...
User Function RLFINR01()   
	...
	Local cPict         	:= ""
	Local titulo       		:= "Relatório Financeiro Titulos por Periodo/Vcto COM MATTER E SEGMENTO"
	Local nLin         		:= 80
	Local Cabec1       		:= ""
	Local Cabec2       		:= ""
	Local imprime      		:= .T.
	...
	Private limite          := 220
	Private tamanho         := "G"
	Private nomeprog        := "RLFINR01" // Coloque aqui o nome do programa para impressao no cabecalho
	Private nTipo           := 18
	Private aReturn         := { "Zebrado", 1, "Administracao", 2, 2, 1, "", 1}
	Private nLastKey        := 0
	Private cPerg       	:= "RLFINR01"
	Private cbtxt      		:= Space(10)
	Private cbcont     		:= 00
	Private CONTFL     		:= 01
	Private m_pag      		:= 01
	Private wnrel      		:= "RLFINR01" // Coloque aqui o nome do arquivo usado para impressao em disco
	Private cString 		:= "SE1" 
	Private a_ITEM			:= {} 

	//--------------------------------------------------------------------------------------------	
	//MsgINFO('Esse relatório precisa ser Validado e Encerrado chamado ID 30705','VR 25-09a')
	MsgINFO('Esse relatório FOI SUBSTITUIDO PELO AGING VR2 - ID 30977','VR 29-01a')
	Return Nil 
	//--------------------------------------------------------------------------------------------
...
// ---- 1o while --------------------------------------------------------------------------------------
	...
	While RLTFIN02->(!EOF())  // While para para o Cabecalho
		...
		@ nLin, a_Cols[12] PSAY Transform(RLTFIN02->SALDO,"@E 999,999,999.99")
		...
		n_Current	:= IIF(n_DTRef<=0,RLTFIN02->SALDO,0)
		n_PVenc1	:= IIF(n_DTRef>=1   .And. n_DTRef<=30,RLTFIN02->SALDO,0)	
		n_PVenc2	:= IIF(n_DTRef>=31  .And. n_DTRef<=90,RLTFIN02->SALDO,0)
		n_PVenc3	:= IIF(n_DTRef>=91  .And. n_DTRef<=180,RLTFIN02->SALDO,0)
		n_PVenc4	:= IIF(n_DTRef>=181 .And. n_DTRef<=360,RLTFIN02->SALDO,0)	
		n_PVenc5	:= IIF(n_DTRef>360,RLTFIN02->SALDO,0)
...
// ---- Fim - 1o while --------------------------------------------------------------------------------------
	...
	If MSGYESNO( "Deseja Exportar o Relatório para Excel?", "Contas a Receber - Periodo" )
		Processa( {|| U_RLFINR11() } , "Exportando Relatório, aguarde...")
	EndIf
...
c_QryAux +="AND E1_SALDO <> 0 AND SE1.D_E_L_E_T_ = ''																					"	+ c_EOL

//If MV_PAR17 == 2
//	c_QryAux +="AND E1_TIPO NOT IN ('NCC', 'RA', 'RC')									   												"	+ c_EOL
//EndIf

c_QryAux +="																			   												"	+ c_EOL
c_QryAux +="ORDER BY VENCREAL														   					   								"	+ c_EOL

IF Select("RLTFIN02") > 0     
	RLTFIN02->(dbCloseArea())
EndIf

MemoWrite("RLTFIN02.SQL", c_QryAux)
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿±±
±±³Programa  ³ RLFINR1A  |  Autor ³ Tiago Dias (Focus Consultoria)  | 	Data ³ 18/09/15      ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
...
//Private c_Perg  := "RLTFIN09"  // Grupo de pergunta que sera criado no SX1
//Private l_Ret	:= .F.

/*Tela Inicial
Define MsDialog oDlg Title 'FTI - Contas a Receber - por Período' From 7,10 To 15,55	Of oMainWnd              
@ 15 ,15  Say	'Esta Rotina tem como objetivo exportar o Relatório Financeiro'  	Of oDlg	Pixel  
@ 25 ,15  Say	'de acordo com os Parametros digitados pelo Usuário para Excel.'  	Of oDlg Pixel  
...
If l_Ret 
	...
	While RLTFIN02->(!EOF()) 
		...
		c_Titulo	:= RIGHT(ALLTRIM(RLTFIN02->NUMTIT),6)
		...
		n_Valor		:= RLTFIN02->VALOR	
		n_Saldo		:= RLTFIN02->SALDO
		...
		oExcel:AddRow ( "Relatório_Financeiro" , "Relatório_Financeiro_"+c_DataQ, {c_Titulo,c_Prefixo,c_Tipo,c_CodCli,c_NomeCli,c_Naturez, c_Segment, c_Matter,DTOC(STOD(c_Emissao)),DTOC(STOD(c_Vencto)),n_Valor,n_Saldo,n_Current,n_PVenc1,n_PVenc2,n_PVenc3,n_PVenc4,n_PVenc5} )
	...
	//- Linha TOTAIS
		oExcel:AddRow ( "Relatório_Financeiro" , "Relatório_Financeiro_"+c_DataQ, {"","","","","","","","","","Totais: ",n_TotVal,n_TotGer,n_TotCurr,n_TotPV1,n_TotPV2,n_TotPV3,n_TotPV4,n_TotPV5} )
	
		MsUnLock()
	
		c_Arq := UPPER("RLFINR01_")+ DtoS(Date()) +"_"+ StrTran(Time(),":","")
